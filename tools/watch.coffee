...
#-------------------------------------------------------------------------------
# Takes an object with a `files` and `run` property.  When a file matching
# the specified file specs changes, the `run` function will be run.
#-------------------------------------------------------------------------------
global.watch = ({files, run}) ->
...
runWatch = (fileSpecs, run, fileName, watchers=[]) ->
  ...
  run fileName if watchers.tripped
