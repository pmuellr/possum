...
There are three types of programs communicating when using possum:
...
An *agent* connects with *targets*, and to the *server*.  An *agent* may be
a program running along-side a *target*, or may be code running inside a
*target*.  It acts as an HTTP client to the *server*.
...
A *server* is an HTTP server that *agents* and *clients* connect to, in order
to interact with each other. It acts as an HTTP server for *agents* and
*clients*.
...
`agentID` and `clientID` are values provided by the possum server when the
...
The agent sends this event when a target disconnects.
